...
DEFAULT_HEADERS = {"User-Agent": os.getenv("USER_AGENT", "Mozilla/5.0 (compatible; NicoTagMonitor/1.0)")}
...
def notify_teams(message: str) -> bool:
    ...
    if not webhook:
        return False
    try:
        r = requests.post(webhook, json={"text": message}, timeout=15)
    ...
    for vid in video_ids:
        try:
            ...
            if removed:
                ...
                sent = notify_line(msg) or notify_teams(msg)
                if not sent:
                    logging.warning("通知先が設定されていないか、送信に失敗しました。")
            else:
